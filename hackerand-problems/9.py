...
def fun(s):
    ...
    username, resto = s.split("@")
    

    if "." not in resto or resto.count(".") != 1:
        return False
    
    website, extension = resto.split(".")
    
    
    if not re.fullmatch(r"[A-Za-z0-9_-]+", username):
        return False
    if not re.fullmatch(r"[A-Za-z0-9]+", website):
        return False
    if not re.fullmatch(r"[A-Za-z]+", extension) or len(extension) > 3:
        return False
